...
# 项目路径   （找到自己的项目路径）
project_path = '/Users/tonyreet/Documents/wb/BananaReading'
# 项目project.pbxproj文件路径 需要更新配置文件中的类名 （找到自己的项目project.pbxproj路径）
pbxpro_path = '/Users/tonyreet/Documents/wb/BananaReading/BananaReading/BananaReading.xcodeproj'
...
def file_rename(file_path):
    ...
    new_path = os.path.join(root_path, filename.replace(old_str, new_to_str) + filetype)    # 拼接新路径
    os.renames(file_path, new_path)             # 文件重命名
    return filename.replace(old_str, new_to_str)
...
for (root, dirs, files) in os.walk(project_path):
    for file_name in files:
        if old_str in file_name and file_name.endswith(suf_set):
